...
namespace LMobile.Gen3LicenseManagement.Dao.Services {
	internal class ModuleDao : BaseDao, IModuleDao {
		...
		public Module GetModuleByName(string moduleName, bool includeInactive = false) {
			if (string.IsNullOrEmpty(moduleName)) return null;

			var query = Session.Query<Module, ModuleMapping>();
			if (includeInactive) {
				query.Where(s => s.Main.ModuleName == moduleName);
			} else {
				query.Where(s => s.Main.ModuleName == moduleName & s.Main.IsActive == true);
			}
			return query.ReadFirst();
		}

		public ModuleProperty GetModulePropertyByName(string propertyName, bool includeInactive = false) {
			if (string.IsNullOrEmpty(propertyName)) return null;

			var query = Session.Query<ModuleProperty, ModulePropertyMapping>();
			if (includeInactive) {
				query.Where(s => s.Main.PropertyName == propertyName);
			} else {
				query.Where(s => s.Main.PropertyName == propertyName & s.Main.IsActive == true);
			}
			return query.ReadFirst();
		}
	}
