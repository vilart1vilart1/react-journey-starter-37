namespace Customer.Ramseier.Database
{
	...
	[Migration(20220408143205)]
	public class ExtendViewDboTurnoverByExtrapolatedCurrentYear : Migration
	{
			...
			Database.ExecuteNonQuery(
				@"ALTER VIEW dbo.Turnover AS
					SELECT
						...
						, MIN(tcy.Total)/MONTH(GETDATE())*12 AS ExtrapolatedTotalCurrentYear
						, MIN(tpy.Total) AS TotalPreviousYear
						, MIN(tppy.Total) AS TotalPrePreviousYear
						, MIN(tmty.Total) AS TotalMinusThreeYears
						, (MIN(tcy.Total)/MONTH(GETDATE())*12) - MIN(tpy.Total) AS [Difference]
						, ot.ModifyDate
						, ot.CreateDate
						, ot.ModifyUser
						, ot.CreateUser
