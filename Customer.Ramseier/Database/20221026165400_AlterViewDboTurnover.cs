namespace Customer.Ramseier.Database
{
	...
	[Migration(20221026165500)]
	public class _AlterViewDboTurnover : Migration
	{
			...
			Database.ExecuteNonQuery(
                @"ALTER VIEW [dbo].[Turnover] AS
					SELECT
						...
						, coalesce(MIN(tcy.Total)/MONTH(GETDATE())*12,0) AS ExtrapolatedTotalCurrentYear
						...
						, coalesce(MIN(tcy.Total)/MONTH(GETDATE())*12,0) - coalesce(MIN(tpy.Total),0) AS [Difference]
						, ot.ModifyDate
						, ot.CreateDate
						, ot.ModifyUser
						, ot.CreateUser
