...
import { Eye, MapPin, Monitor, Calendar, Globe, ChevronLeft, ChevronRight, RefreshCw, Search, Filter } from 'lucide-react';
...
const VisitorsTable = () => {
  ...
  const getReferrerBadge = (referrer?: string) => {
    ...
    // Try to extract hostname from URL, with proper error handling
    try {
      const url = new URL(referrer);
      return <Badge variant="outline" className="truncate max-w-24">{url.hostname}</Badge>;
    } catch (error) {
      // If URL construction fails, check if it looks like a domain
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-gray-50/50">
                      <TableHead className="font-semibold text-gray-700">Adresse IP</TableHead>
                      <TableHead className="font-semibold text-gray-700">Page Visitée</TableHead>
                      ...
                      <TableHead className="font-semibold text-gray-700">Date de Visite</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between p-4 bg-gray-50/50 border-t border-gray-100">
                  <div className="text-sm text-gray-600">
                    Page {currentPage} sur {totalPages} • {filteredVisitors.length} visiteurs affichés
                  </div>
                  <div className="flex items-center space-x-2">
                    <Button
                      variant="outline"
