		...
		BEGIN TRY
			...
			BEGIN
				...
				WHEN NOT MATCHED BY SOURCE 
					...
					AND [target].[ContactId]  in (select contactkey from sms.serviceorderhead where contactkey = [target].ContactId
					And [status] <> ('Completed' or 'PartiallyCompleted')
					THEN
					UPDATE SET 
						[IsActive] = 0
