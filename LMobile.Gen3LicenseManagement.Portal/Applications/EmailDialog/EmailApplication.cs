...
namespace LMobile.Gen3LicenseManagement.Portal.Applications.EmailDialog {
	...
	class EmailApplication : BaseApplication {
		...
		// New method to copy email content to clipboard
		public void CopyEmailToClipboard() {
			try {
				if (!string.IsNullOrEmpty(this.ContractNoEmail)) {
					Clipboard.SetText(this.ContractNoEmail);
					// Show a brief message or feedback that copying was successful
					ShowMessage(Resources.@CopySuccessful(), "Info");
					// Close the dialog after copying
					SendResponse(Responses.NO);
				}
			} catch (Exception ex) {
				Console.WriteLine($"Failed to copy to clipboard: {ex.Message}");
				ShowMessage(Resources.@CopyFailed(), "Error");
			}
		}

		private void ShowMessage(string message, string title) {
			// Simple helper to show a message
			MessageBox.Show(message, title);
		}

		// Keep this method for future use but it's not currently used
