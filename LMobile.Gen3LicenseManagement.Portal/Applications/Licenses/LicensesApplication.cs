...
namespace LMobile.Gen3LicenseManagement.Portal.Applications.Licenses {
	...
	class LicensesApplication : BaseApplication {
				...
				if (this.CurrentProject.ModuleProperties == null || this.CurrentProject.ModuleProperties.Count == 0 && (this.ModuleProperties != null && this.ModuleProperties.Count > 0)) {
					using (var transaction = Session.BeginTransaction()) {
						foreach (var p in this.ModuleProperties) {
							...
							// Enhanced logging with detailed module information
							string logMessage = string.Format("Module {0} added by '{1}'", newProjectModule, Client.CurrentPrincipal.Identity.Name);
							string longMessage = string.Format("Module Addition Details:\nModule Name: {0}\nModule Description: {1}\nProperty Name: {2}\nProperty Description: {3}\nProject: {4}\nAdded by: {5}",
								!string.IsNullOrEmpty(mySelectedNewModule.ModuleName) ? mySelectedNewModule.ModuleName : "N/A",
								!string.IsNullOrEmpty(mySelectedNewModule.Description) ? mySelectedNewModule.Description : "N/A",
								!string.IsNullOrEmpty(p.PropertyName) ? p.PropertyName : "N/A",
								!string.IsNullOrEmpty(p.Description) ? p.Description : "N/A",
								!string.IsNullOrEmpty(CurrentProject.Description) ? CurrentProject.Description : "N/A",
								Client.CurrentPrincipal.Identity.Name);

							LicenseDao.LogEntry(CurrentCustomer.ID, CurrentProject.ID, MessageTypes.ProjectModified, logMessage, longMessage);
		...
		public void SaveAllProjectModuleProperties() {
			...
			HasAllChanges = false;
			LicenseDao.LogEntry(CurrentCustomer.ID, CurrentProject.ID, MessageTypes.ProjectModified, "All Project module saved by '" + Client.CurrentPrincipal.Identity.Name + "'.");
		}
		...
		public void SaveProjectModuleProperty(ProjectModuleProperty pMod) {
			...
			if (pMod == null) {
				...
				// Enhanced logging with detailed module information
				string logMessage = string.Format("Module {0} added by '{1}'", SearchModuleName, Client.CurrentPrincipal.Identity.Name);
				string longMessage = string.Format("Module Addition Details:\nSearch Term: {0}\nProperty Name: {1}\nProperty Description: {2}\nLicense Count: {3}\nTest License Count: {4}\nExpiry Date: {5}\nProject: {6}\nAdded by: {7}",
					!string.IsNullOrEmpty(SearchModuleName) ? SearchModuleName : "N/A",
					!string.IsNullOrEmpty(SelectedNewModuleProperty.PropertyName) ? SelectedNewModuleProperty.PropertyName : "N/A",
					!string.IsNullOrEmpty(SelectedNewModuleProperty.Description) ? SelectedNewModuleProperty.Description : "N/A",
					NewModulePropertyLicenceCount,
					NewModulePropertyTestLicenceCount,
					NewModulePropertyExpiryDate.ToString("yyyy-MM-dd"),
					!string.IsNullOrEmpty(CurrentProject.Description) ? CurrentProject.Description : "N/A",
					Client.CurrentPrincipal.Identity.Name);

				LicenseDao.LogEntry(CurrentCustomer.ID, CurrentProject.ID, MessageTypes.ModuleAdded, logMessage, longMessage);
		...
		public void DeleteProjectModuleProperty(ProjectModuleProperty pMod) {
			// Get module property details before deletion for enhanced logging
			var moduleProperty = pMod.ModuleProperty ?? ModuleDao.GetModuleProperty(pMod.ModulePropertyID);

			Session.Delete(pMod);

			// Enhanced logging with detailed module information
			string logMessage = "Project module deleted by '" + Client.CurrentPrincipal.Identity.Name + "'";
			string longMessage = string.Format("Module Deletion Details:\nProperty Name: {0}\nProperty Description: {1}\nProject: {2}\nDeleted by: {3}",
				moduleProperty != null && !string.IsNullOrEmpty(moduleProperty.PropertyName) ? moduleProperty.PropertyName : "N/A",
				moduleProperty != null && !string.IsNullOrEmpty(moduleProperty.Description) ? moduleProperty.Description : "N/A",
				!string.IsNullOrEmpty(CurrentProject.Description) ? CurrentProject.Description : "N/A",
				Client.CurrentPrincipal.Identity.Name);
