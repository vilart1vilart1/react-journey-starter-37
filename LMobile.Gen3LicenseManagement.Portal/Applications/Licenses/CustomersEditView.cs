	...
	class CustomersEditView : ApplicationView<BaseView, LicensesApplication> {
		...
		protected override void Render(BaseView master) {
			...
			this.AddIteration(Projects, () => {
				projects.AddRow(row => {
					bool isMoreThanThreeMonths = false;
		...
		.BindCaption(Projects,
					 project => {
						 var result = FormatModifyDate(project);
						 isMoreThanThreeMonths = result.IsMoreThanThreeMonths;
						 return result.FormattedDate;
					 })
		.BindStyle(() => ClassicStyleSheet.W1in2Remainder +
						(isMoreThanThreeMonths ? ClassicStyleSheet.Negative : null));
