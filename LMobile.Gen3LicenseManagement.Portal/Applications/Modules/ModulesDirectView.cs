	...
	class ModulesDirectView : ApplicationView<BaseView, ModulesDirectApplication> {
		...
		protected override void Render(BaseView master) {
			...
			editorFields.AddRow(row => {
				row.AddLabel().SetCaption(Resources.ModuleName() + ":");
				row.AddTextBox()
			...
			packagesTable.AddRow(row => {
				...
				row.AddLabel().SetCaption(Resources.ID()).SetStyle(new Style { Width = new Length(60, In.Pixels) });
				row.AddLabel().SetCaption(Resources.ModuleName()).SetStyle(new Style { Width = new Length(200, In.Pixels) });
				row.AddLabel().SetCaption(Resources.Description()).SetStyle(ClassicStyleSheet.WRemainder);
				row.AddLabel().SetCaption(Resources.Action()).SetStyle(new Style { Width = new Length(140, In.Pixels) });
			});
